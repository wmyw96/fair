...
def fair_nn_gumbel_regression(features, responses, validset, testset, hyper_gamma=10, depth=1, width=64,
						learning_rate=1e-3, niters=50000, niters_d=5, niters_g=1, gate_samples=20,
	...
	# build the gumbel gate
	fairnn_var = GumbelGate(dim_x)
	optimizer_var = optim.Adam(fairnn_var.parameters(), lr=learning_rate)

	# build predictor class G
	fairnn_g = FairNeuralNetwork(dim_x, depth, width, res_connect=True)
	...
	optimizer_ds = []
	for e in range(num_envs):
		fairnn_d = FairNeuralNetwork(dim_x, depth, 2*width, res_connect=True)
	...
	for it in range(niters):
		...
		if it % eval_iter == 0:
			...
			for k in range(gate_samples):
				gate = fairnn_var.generate_mask(tau)
